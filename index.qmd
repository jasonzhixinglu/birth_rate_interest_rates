...
| $\beta$ | 0.9634 | Discount factor (calibrated to $r_\text{init} = 3.0\%$) |
| $\theta$ | 2.0 | Inverse EIS |
| $\gamma$ | 0.015 | TFP growth rate |
| $J$ | 80 | Lifespan (periods) |
| $\chi$ | 20 | Age entering labour market |
| $\psi$ | 65 | Retirement age |

## Results

### Germany Baseline

Starting from 3.0 percent in the early 1980s, the model-implied rate falls to about 0.8 percent by 2031 — a peak-to-trough decline of roughly two percentage points — before recovering to a new balanced-growth-path level of approximately 1.6 percent. The brief overshoot below the new equilibrium reflects the lag between peak saving pressure and the onset of dissaving by the large pre-shock cohort. A retirement age of 65 (working span 20–65) shortens the retirement phase relative to a retirement age of 60, weakening the saving motive, so the decline is milder and the trough later and modestly positive.

![Germany baseline: model-implied interest rate, 1950–2050.](figures/germany_baseline.png){fig-align="center" width="80%"}

### Cross-Country Comparison

Countries with an earlier fertility decline trough sooner, and those with a sharper decline trough deeper relative to their starting point: the United States troughs earliest (2030) and shallowest, while China — with a later and larger fertility drop — troughs latest (2041). All cross-country differences are captured using only country-specific cohort-size data and a common structural parameterisation.

![Cross-country comparison: model-implied real interest rates.](figures/cross_country.png){fig-align="center" width="80%"}

## Conclusion

The post-1970 fertility collapse accounts for approximately two percentage points of the observed decline in real interest rates across advanced economies, with a trough below one percent around 2031. After that point, the gradual exit of the large pre-shock cohort from the saving pool allows rates to recover toward a new, lower balanced-growth-path level. The current era of depressed rates is therefore partly transitional, and policy frameworks premised on their permanence may require revision.
